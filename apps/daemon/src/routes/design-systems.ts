...
export interface RegisterDesignSystemRoutesDeps extends RouteDeps<'db' | 'http' | 'paths' | 'projectFiles' | 'projectStore'> {
  designSystems: {
...
export function registerDesignSystemRoutes(app: Express, ctx: RegisterDesignSystemRoutesDeps) {
  const { db } = ctx;
  const requireLocalDaemonRequest = ctx.http.requireLocalDaemonRequest as RequestHandler;
  ...
  app.get('/api/design-systems/refero/status', async (_req, res) => {
    if (!ctx.refero) return res.status(404).json({ error: 'Refero design-system sync is not configured' });
    res.json({ refero: ctx.refero.status() });
  });

  app.post('/api/design-systems/refero/sync', requireLocalDaemonRequest, async (_req, res) => {
    if (!ctx.refero) return res.status(404).json({ error: 'Refero design-system sync is not configured' });
    try {
      const result = await ctx.refero.run(true);
      res.json({ refero: ctx.refero.status(), result });
    } catch (err) {
      res.status(500).json({ error: String(err), refero: ctx.refero.status() });
    }
  });
