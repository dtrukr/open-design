...
export function createDesignSystemServerServices({
  ...
  paths: {
    ...
    REFERO_DESIGN_SYSTEMS_DIR: string;
  };
  skills: {
  ...
  async function listAllDesignSystems() {
    ...
    let refero: DesignSystemSummary[] = [];
    try {
      refero = (await designSystems.listDesignSystems(paths.REFERO_DESIGN_SYSTEMS_DIR, {
        source: 'installed',
        isEditable: false,
      ...
      // Refero sync is optional and may not have populated its directory yet.
    }
    try {
    ...
    const referoUnique = refero.filter((s) => !seen.has(s.id));
    for (const system of referoUnique) seen.add(system.id);
    return [
      ...installed
  ...
  async function readAvailableDesignSystem(id: string) {
    ...
    return (
      ...
      ?? (await designSystems.readDesignSystem(paths.REFERO_DESIGN_SYSTEMS_DIR, id))
  ...
  async function readAvailableDesignSystemPackageInfo(id: string) {
    ...
    return (
      ...
      ?? (await designSystems.readDesignSystemPackageInfo(paths.REFERO_DESIGN_SYSTEMS_DIR, id))
  ...
  async function readAvailableDesignSystemStaticFile(id: string, filePath: string) {
    ...
    return (
      ...
      ?? (await designSystems.readDesignSystemStaticFile(paths.REFERO_DESIGN_SYSTEMS_DIR, id, filePath))
